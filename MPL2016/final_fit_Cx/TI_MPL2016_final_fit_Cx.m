function perf = TI_MPL2016_final_fit_Cx(param, sim)
...
for i_cond_S2 = 1:length(sim.S2_list)
    ...
    conf_Cx_mean(i_cond_S2)   = mean(conff);
    rating_Cx_mean(i_cond_S2) = mean(ratingf);

    rating_Cx_rS1_corr(i_cond_S2)   = mean(ratingf(accf==1 & respf==0));
    rating_Cx_rS1_incorr(i_cond_S2) = mean(ratingf(accf==0 & respf==0));
    rating_Cx_rS2_corr(i_cond_S2)   = mean(ratingf(accf==1 & respf==1));
    rating_Cx_rS2_incorr(i_cond_S2) = mean(ratingf(accf==0 & respf==1));

    % compute meta-d'
    [nR_S1, nR_S2] = trials2counts(stimIDf, respf, ratingf, 4, 1);
    fit = fit_meta_d_MLE(nR_S1, nR_S2);

    md_Cx(i_cond_S2) = fit.meta_da;

    fit = fit_rs_meta_d_MLE(nR_S1, nR_S2);

    md_Cx_rS1(i_cond_S2) = fit.meta_da_rS1;
    md_Cx_rS2(i_cond_S2) = fit.meta_da_rS2;
