...
param.tau   = 26;  % derived from md_Cd_search fit
...
for i_rep = 1:sim.nreps
    perf = TI_MPL2016_final_fit_Cd(param, sim);
    ...
    md_Cd_rS1(:, i_rep) = perf.md_Cd_rS1';
    md_Cd_rS2(:, i_rep) = perf.md_Cd_rS2';

    % mean conf rating
    rating_Cd(:, i_rep)            = perf.rating_Cd_mean';
    rating_Cd_rS1_corr(:, i_rep)   = perf.rating_Cd_rS1_corr';
    rating_Cd_rS1_incorr(:, i_rep) = perf.rating_Cd_rS1_incorr';
    rating_Cd_rS2_corr(:, i_rep)   = perf.rating_Cd_rS2_corr';
    rating_Cd_rS2_incorr(:, i_rep) = perf.rating_Cd_rS2_incorr';
...
md_Cd     = mean(md_Cd, 2);
md_Cd_rS1 = mean(md_Cd_rS1, 2);
md_Cd_rS2 = mean(md_Cd_rS2, 2);

rt       = mean(rt, 2);
rtmin    = mean(rtmin, 2);
rtmax    = mean(rtmax, 2);

rating_Cd            = mean(rating_Cd, 2);
rating_Cd_rS1_corr   = mean(rating_Cd_rS1_corr, 2);
rating_Cd_rS1_incorr = mean(rating_Cd_rS1_incorr, 2);
rating_Cd_rS2_corr   = mean(rating_Cd_rS2_corr, 2);
rating_Cd_rS2_incorr = mean(rating_Cd_rS2_incorr, 2);
...
filename = 'TI_MPL2016_final_fit_Cd';

if savedata
    results = v2struct(d, md_Cd, md_Cd_rS1, md_Cd_rS2, rt, rtmin, rtmax, ...
...
figure; hold on;
plot(d, md_Cd, 'ko-', 'LineWidth',lw)
plot(d, md_Cd_rS1, 'r^-', 'LineWidth',lw)
plot(d, md_Cd_rS2, 'bv-', 'LineWidth',lw)
...
figure; 
subplot(2,1,1); hold on;
plot(1:5, rating_Cd_rS1_corr, 'ro-', 'LineWidth', lw)
plot(1:5, rating_Cd_rS1_incorr, 'rx-', 'LineWidth', lw)
...
subplot(2,1,2); hold on;
plot(1:5, rating_Cd_rS2_corr, 'bo-', 'LineWidth', lw)
plot(1:5, rating_Cd_rS2_incorr, 'bx-', 'LineWidth', lw)
