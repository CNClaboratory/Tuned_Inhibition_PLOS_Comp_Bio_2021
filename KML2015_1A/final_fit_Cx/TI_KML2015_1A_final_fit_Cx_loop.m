function TI_KML2015_1A_final_fit_Cx_loop(alpha_lowPE, alpha_highPE)
...
% values for tau derived from md_Cx_search fit
switch alpha_lowPE
    case 0.1
        param.tau = 48;
    case 0.3
...
for i_rep = 1:sim.nreps
    perf = TI_KML2015_1A_final_fit_Cx(param, sim);

    d(:, :, i_rep)         = perf.d;              % d'
    md_Cx(:, :, i_rep)     = perf.md_Cx;          % meta-d'
    rating_Cx(:, :, i_rep) = perf.rating_Cx_mean; % mean conf rating
...
d         = squeeze( mean(d, 3) );
md_Cx     = squeeze( mean(md_Cx, 3) );
rating_Cx = squeeze( mean(rating_Cx, 3) );
...
filename = ['TI_KML2015_1A_final_fit_Cx_alpha_lowPE=' num2str(alpha_lowPE) '_alpha_highPE=' num2str(alpha_highPE)];

if savedata
    results = v2struct(d, md_Cx, rating_Cx, ...
...
style = {'bv-', 'r^-'};
for i_PE = 1:2
    plot(d(i_PE,:), rating_Cx(i_PE,:), style{i_PE})
end
...
style = {'bv-', 'r^-'};
for i_PE = 1:2
    plot(d(i_PE,:), md_Cx(i_PE,:), style{i_PE})
end
