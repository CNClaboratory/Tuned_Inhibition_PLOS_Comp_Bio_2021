function perf = TI_KML2015_1A_final_fit_Cd(param, sim)
...
% in KML 2015 expt 1A data set
prating = [0.2552, 0.3244, 0.2764, 0.1440];
...
for i_cond_PE = 1:2
    for i_cond_diff = 1:3
       ...
       % compute accuracy and RT
        ...
        conf_Cd_mean(i_cond_PE, i_cond_diff)   = mean(conff);
        rating_Cd_mean(i_cond_PE, i_cond_diff) = mean(ratingf);

        % compute meta-d'
        [nR_S1, nR_S2] = trials2counts(stimIDf, respf, ratingf, 4, 1);
        fit = fit_meta_d_MLE(nR_S1, nR_S2);

        md_Cd(i_cond_PE, i_cond_diff) = fit.meta_da;
...
perf   = v2struct(stimID, S_PE, S_NE, cond_PE, cond_diff, responded, responded2, resp, rt, ...
                  conf_Cx, conf_Cd, rating_Cd, presp, pcorr, ...
